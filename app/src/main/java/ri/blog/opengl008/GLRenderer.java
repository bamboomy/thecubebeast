...
import com.bamboomy.thecubebeast.game.Pictures;
...
public class GLRenderer implements Renderer {
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		GLES20.glAttachShader(riGraphicTools.sp_Text, fshadert); // add the
																	// fragment
																	// shader to
																	// program
		GLES20.glLinkProgram(riGraphicTools.sp_Text); // creates OpenGL ES
														// program executables
	...
	public void SetupImage() {
		...
		int id = mContext.getResources().getIdentifier("drawable/font", null,
				mContext.getPackageName());
		
		//Bitmap bmp = BitmapFactory.decodeResource(mContext.getResources(), id);
		
		Bitmap bmp = Pictures.decodeSampledBitmapFromResource(id, 200, 200, mContext);
	...
	class Sprite {
		...
		public float[] getTransformedVertices() {
			...
			return new float[] { one.x, one.y, 0.0f, two.x, two.y, 0.0f,
					three.x, three.y, 0.0f, four.x, four.y, 0.0f, };
		}
	}
}
