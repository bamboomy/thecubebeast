...
public class TextManager {
	...
	public static int[] l_size = { 36, 29, 30, 34, 25, 25, 34, 33, 11, 20, 31,
			24, 48, 35, 39, 29, 42, 31, 27, 31, 34, 35, 55, 35, 31, 27, 30, 26,
			28, 26, 31, 28, 28, 28, 29, 29, 14, 24, 30, 18, 26, 14, 14, 14, 25,
			28, 31, 0, 0, 38, 39, 12, 36, 34, 0, 0, 0, 38, 0, 0, 0, 0, 0, 0 };
	...
	public void AddCharRenderInformation(float[] vec, float[] cs, float[] uv,
			short[] indi) {
		...
		// We should add the colors, so we can use the same texture for multiple
		// effects.
	...
	public void Draw(float[] m) {
		...
		int mPositionHandle = GLES20.glGetAttribLocation(
				riGraphicTools.sp_Text, "vPosition");
	...
	private void convertTextToTriangleInfo(TextObject val) {
		...
		for (int j = 0; j < text.length(); j++) {
			...
			// Select only the appropriate part of the texture to map it
			// on a rectangle with a corrected size
			float width = (l_size[indx] / 2) * uniformscale;
			float u_width = (l_size[indx] / 2) / RI_TEXT_WIDTH
					* RI_TEXT_UV_BOX_WIDTH;
			...
			colors = new float[] { val.color[0], val.color[1], val.color[2],
					val.color[3], val.color[0], val.color[1], val.color[2],
					val.color[3], val.color[0], val.color[1], val.color[2],
					val.color[3], val.color[0], val.color[1], val.color[2],
					val.color[3] };
